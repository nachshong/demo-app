...
export class PostsService {
  ...
  getPosts(): Observable<Array<Post>> {
    return this.httpClient.get<Array<any>>(API_SOURCE + '/posts')
      .pipe(map(list => list.map(
        s => { return new Post(s.id, s.userId, s.title, s.body, s.version)}
      )));
  }
  ...
  addPost(post: Post): Observable<Post> {
    ...
    return this.httpClient.post(API_SOURCE + '/posts', value).pipe(
      map((s: any) => new Post(s.id, s.userId, s.title, s.body, s.version))
    );
  }

  editUser(post: Post): Observable<Post> {
    var value = this.getRemotePost(post);
    return this.httpClient.put(API_SOURCE + '/posts/' + post.id, value).pipe(
      map((s: any) => new Post(s.id, s.userId, s.title, s.body, s.version))
    );
  }
